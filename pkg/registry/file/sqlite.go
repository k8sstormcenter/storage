...
func NewPool(path string, size int) *sqlitemigration.Pool {
	...
	return sqlitemigration.NewPool(path,
		...
		sqlitemigration.Options{
			PoolSize: size,
			// Keep the driver's default 10s busy timeout. A longer wait looked
			// safer against "database is locked" errors, but writers queueing
			// on the single write lock PARK their pool connection for the full
			// wait — with a 60s timeout a write burst drained the whole pool
			// and read requests died on "acquisition timed out", starving the
			// ContainerProfile pipeline cluster-wide. Lock errors are
			// retriable for every writer (the node-agent queue requeues, the
			// consolidator re-runs); fail fast and free the connection.
		})
}
...
func HostKeysToPath(prefix, root, kind, hostID, cloudAccountIdentifier, region, name string) string {
	return fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s", prefix, root, kind, hostID, cloudAccountIdentifier, region, name)
}
...
func BuildContainerProfileKey(id armotypes.ProfileIdentifier, kind string) string {
	...
	default:
		return HostKeysToPath("", softwarecomposition.GroupName, kind, id.HostID, id.CloudAccountIdentifier, id.Region, id.Name)
	}
}
...
func ParseContainerProfileKey(key string, hostType armotypes.HostType) (id armotypes.ProfileIdentifier, prefix, root, kind string, err error) {
	...
	default:
		if len(parts) < 7 {
			return id, prefix, root, kind, fmt.Errorf("invalid Host key format: expected 7 parts, got %d", len(parts))
		}
		id.HostID = parts[3]
		id.CloudAccountIdentifier = parts[4]
		id.Region = parts[5]
